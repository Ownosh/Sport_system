...
class CreateUserWindow(QWidget):
    ...
    def submit_form(self):
        ...
        if not dob:
            QMessageBox.warning(self, "Ошибка", "Поле 'Дата рождения' обязательно для заполнения.")
            return
        ...
        try:
            ...
            if hasattr(self, 'photo_file'):
                ...
                if role == "Sportsman":
                    cursor.execute("UPDATE sportsmen SET photo = %s WHERE user_id = %s", (photo_data, user_id))
                elif role == "Trainer":
                    cursor.execute("UPDATE trainers SET photo = %s WHERE user_id = %s", (photo_data, user_id))
