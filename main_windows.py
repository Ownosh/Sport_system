...
from main_buttons import (
    ...
    ProfileWindow,  GroupMembersWindow
)
from group_button import GroupWindow, GroupWindowForTrainers
from sportsman_button import AdminSportsmenWindow, TrainerSportsmenWindow
from trainer_button import TrainerWindow
from report_button import ReportWindow,AthleteReportWindow
...
class AdminWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.materials_button = self.create_button("Доп. материалы", self.open_materials_)  # Новая кнопка
        ...
        top_nav_layout.addWidget(self.materials_button)  # Добавляем кнопку в верхнюю навигацию
    ...
    def open_sportsmen_(self):
        self.open_window(AdminSportsmenWindow)
    ...
    def open_materials_(self):
        self.clear_workspace()
        materials_widget = MaterialsWindow()  # Создайте класс MaterialsWindow для отображения доп. материалов
        self.workspace.layout().addWidget(materials_widget)
...
class AthleteWindow(QWidget):
    ...
    def load_athlete_info(self):
        ...
        if connection:
            ...
            try:
                cursor.execute("""
            SELECT first_name, last_name, birthdate, gender, city, typesport, photo 
            FROM sportsmen 
        ...
        if connection:
            ...
            try:
                cursor.execute("""
                    ...
                    WHERE sportsman_id = (
                        SELECT sportsman_id FROM sportsmen 
                        WHERE user_id = (SELECT user_id FROM users WHERE username = %s)
                    )
                """, (self.current_username,))
                ...
                for recommendation in recommendations:
                    recommendations_layout.addWidget(QLabel(str(recommendation[0])))  # Преобразование в строку
    ...
    def load_dopmaterials(self):
        ...
        if connection:
            cursor = connection.cursor()
            try:
                cursor.execute("SELECT media_id FROM media WHERE sportsman_id = (SELECT sportsman_id FROM sportsmen WHERE user_id = (SELECT user_id FROM users WHERE username = %s))", (self.current_username,))
...
class CoachWindow(QWidget):
    ...
    def display_report(self):
        self.clear_workspace()
        report_window = AthleteReportWindow(self.workspace)
        self.workspace.layout().addWidget(report_window)
    ...
    def open_sportsmen_(self):
        self.open_window(TrainerSportsmenWindow)
...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QListWidget, QFileDialog, QMessageBox
from PyQt5.QtCore import Qt
import os
from change_buttons import get_database_connection  # Используем вашу функцию подключения

class MaterialsWindow(QWidget):
    def __init__(self, sportsman_id):
        super().__init__()
        self.sportsman_id = sportsman_id  # ID спортсмена для привязки файлов
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Кнопки для управления материалами
        button_layout = QHBoxLayout()

        self.upload_button = QPushButton("Загрузить материал")
        ...
        # Список для отображения материалов
        self.materials_list = QListWidget()
        self.materials_list.setSelectionMode(QListWidget.SelectionMode.SingleSelection)
        layout.addWidget(self.materials_list)

        # Загрузка существующих материалов из базы данных
        self.load_materials()

        self.setLayout(layout)

    def upload_material(self):
        """Загружает материал в базу данных."""
        file_dialog = QFileDialog()
        file_path, _ = file_dialog.getOpenFileName(self, "Выберите файл", "", "Изображения (*.png *.jpg *.jpeg);;Видео (*.mp4 *.avi);;Все файлы (*)")

        if file_path:
            file_name = os.path.basename(file_path)
            file_type = "photo" if file_path.lower().endswith((".png", ".jpg", ".jpeg")) else "video"
            ...
            try:
                cursor.execute(
                    "INSERT INTO media (sportsman_id, file_name, file_type, file_data) VALUES (%s, %s, %s, %s)",
                    (self.sportsman_id, file_name, file_type, file_data)
                )
                connection.commit()
                self.materials_list.addItem(file_name)
                QMessageBox.information(self, "Успех", f"Файл {file_name} успешно загружен!")
            except Exception as e:
                QMessageBox.critical(self, "Ошибка базы данных", f"Ошибка при сохранении файла: {e}")
            finally:
                cursor.close()
                connection.close()
    ...
    def delete_material(self):
        ...
        selected_item = self.materials_list.currentItem()
        ...
        if selected_item:
            ...
            try:
                ...
                cursor.execute("DELETE FROM media WHERE sportsman_id = %s AND file_name = %s", (self.sportsman_id, file_name))
                connection.commit()
                self.materials_list.takeItem(self.materials_list.row(selected_item))
                QMessageBox.information(self, "Успех", f"Файл {file_name} успешно удален!")
            except Exception as e:
                QMessageBox.critical(self, "Ошибка базы данных", f"Ошибка при удалении файла: {e}")
            finally:
                cursor.close()
                connection.close()
        else:
    ...
    def load_materials(self):
        """Загружает список материалов спортсмена из базы данных."""
        connection = get_database_connection()
        cursor = connection.cursor()
        try:
            cursor.execute("SELECT file_name FROM media WHERE sportsman_id = %s", (self.sportsman_id,))
            materials = cursor.fetchall()
            for material in materials:
                self.materials_list.addItem(material[0])
        except Exception as e:
            QMessageBox.critical(self, "Ошибка базы данных", f"Ошибка при загрузке материалов: {e}")
        finally:
            cursor.close()
            connection.close()
...
class LoginWindow(QWidget):
    ...
    def check_user_credentials(self):
        ...
        try:
            ...
            query = "SELECT password, role, active FROM users WHERE username = %s"
            ...
            if result:
                ...
                if active == 0:
                    QMessageBox.warning(self, "Ошибка", "Ваш аккаунт не активен. Обратитесь к администратору.")
                    return
